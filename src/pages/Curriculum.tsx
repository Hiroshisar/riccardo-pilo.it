...
function Curriculum() {
  const groupedData: GroupedWork[] = Object.values(
    curriculumWorkData.reduce<Record<number, GroupedWork>>((acc, entry) => {
      if (!acc[entry.year]) {
        acc[entry.year] = {
          year: entry.year,
          items: []
  ...
  return (
    <div className="relative isolate mx-auto mt-10 max-w-5xl px-4">
      <div className="bg-primary-complement absolute top-0 left-1/2 z-0 h-full w-px -translate-x-1/2" />
        ...
        {groupedData.map((group, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="text-subtitle text-3xl font-bold">
                  {group.year}
                </div>
              </div>

              <div className="bg-primary absolute top-1/2 left-1/2 hidden h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full md:block" />
            </div>
          );
        })}
      </div>
